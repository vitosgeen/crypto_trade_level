...
func (s *SQLiteStore) ListLevels(ctx context.Context) ([]*domain.Level, error) {
	query := `SELECT id, exchange, symbol, level_price, side, base_size, leverage, margin_type, cool_down_ms, stop_loss_at_base, stop_loss_mode, disable_speed_close, max_consecutive_base_closes, base_close_cooldown_ms, take_profit_pct, take_profit_mode, is_auto, auto_mode_enabled, ignore_sentiment_filter, source, created_at FROM levels`
...
func (s *SQLiteStore) GetLevelsBySymbol(ctx context.Context, symbol string) ([]*domain.Level, error) {
	query := `SELECT id, exchange, symbol, level_price, side, base_size, leverage, margin_type, cool_down_ms, stop_loss_at_base, stop_loss_mode, disable_speed_close, max_consecutive_base_closes, base_close_cooldown_ms, take_profit_pct, take_profit_mode, is_auto, auto_mode_enabled, ignore_sentiment_filter, source, created_at FROM levels WHERE symbol = ?`
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&l.TakeProfitPct, &l.TakeProfitMode, &l.IsAuto, &l.AutoModeEnabled, &l.IgnoreSentimentFilter, &l.Source, &l.CreatedAt,
		); err != nil {
			return nil, err
		}
		levels = append(levels, &l)
